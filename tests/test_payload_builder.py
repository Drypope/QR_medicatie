...
def test_build_payload_single_item():
    ...
    assert payload == "$2037--A1"


def test_build_payload_repeats_segments_for_quantity():
    payload = build_payload([
        SelectedItem(unique_id="A1", product_name="Alpha", quantity=2, product_class="A"),
    ])
    assert payload == "$2037--A1$2037--A1"
...
def test_build_payload_ignores_zero_quantities():
    ...
    assert payload == "$2037--B1$2037--B1"
...
def test_build_payload_is_deterministic_by_class_then_name_then_id():
    ...
    assert payload == "$2037--A1$2037--A1$2037--A1$2037--Z9"
